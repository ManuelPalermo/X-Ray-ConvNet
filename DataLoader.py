...
class DataLoader:
	def __init__(self, ntrain, nval, ntest, batch_size=64, img_shape=(128,128,1), undersample=True, augment_data=True, shuffle=True, plot_distribuition=True):
		self.ntrain       = ntrain
		self.nval         = nval
		self.ntest        = ntest
	...
	def load_train_data(self):
		...
		train_labels = np.vstack(train_data.apply(lambda x: max(x[1].split())).as_matrix()).astype(np.int8)[:self.ntrain]
		if self.undersample:
			...
			train_images = train_data.apply(lambda x: '../database_preprocessed/' + x[0]).as_matrix()[samples_to_train]
		else:
			train_images = train_data.apply(lambda x: '../database_preprocessed/' + x[0]).as_matrix()[:self.ntrain]
		return (train_images, train_labels)

	def load_validation_data(self):
		val_data = pd.read_csv('../dataset/val_1.txt', header=None, index_col=None)[0].str.split(' ', 1)
		val_labels = np.vstack(val_data.apply(lambda x: max(x[1].split())).as_matrix()).astype(np.int8)[:self.nval]
		if self.undersample:
			...
			val_images = val_data.apply(lambda x: '../database_preprocessed/' + x[0]).as_matrix()[samples_to_val]
		else:
			val_images = val_data.apply(lambda x: '../database_preprocessed/' + x[0]).as_matrix()[:self.nval]
		return (val_images, val_labels)

	def load_test_data(self):
		test_data = pd.read_csv('../dataset/test_1.txt', header=None, index_col=None)[0].str.split(' ', 1)
		test_labels = np.vstack(test_data.apply(lambda x: max(x[1].split())).as_matrix()).astype(np.int8)[:self.ntest]
		test_images = test_data.apply(lambda x: '../database_preprocessed/' + x[0]).as_matrix()[:self.ntest]
