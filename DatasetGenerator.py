...
class DataGenerator(keras.utils.Sequence):
	...
	def __init__(self, images_paths, labels, batch_size=64, image_dimensions = (128,128,1), shuffle=True, augment=False):
		self.labels       = labels
	...
	def _data_generation(self, batch_images, batch_labels):
		...
		for i, (img_path, img_label) in enumerate(zip(batch_images, batch_labels)):
			images[i] = io.imread(img_path, as_gray=True).reshape((self.dim))
			labels[i] = img_label
		return images, labels
